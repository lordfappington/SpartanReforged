...
| `.MTL` / `MODELS.MTL` | 1 | 5,952 | ordered resource/material declarations | **CONFIRMED binding, TEST/ZBUF/ALPHA controls, and CPU pass order** | validated R5900 data flow proves child type 2 selects TEST/ZBUF and child type 16 selects ALPHA; CLOUD is in final material range; PRIM.ABE remains at VU1 entry 0 | 3 |
...
2. **P0b — minimum attribute semantics: COMPLETE and visually validated.** V2-16 is signed normalized Q4.12 (`raw / 4096`) with zero global bias and source V orientation. V4-8 bytes 0–2 are **LIKELY** vertex color/light modulation; byte 3 is globally `0x80`, consistent with full PS2 alpha. Exact VU routing remains open.
3. **P0c — material semantics:** validated R5900 analysis proves type 2 constructs alpha/depth-test state and type 16 constructs the blend equation. CLOUD's standard-alpha payload, AREF `0x80`, normal depth test, enabled depth writes, and final-range material order are recovered; effective `PRIM.ABE` at VU1 entry 0 remains the bounded blocker.
