...
| `.MTL` / `MODELS.MTL` | 1 | 5,952 | ordered resource/material declarations | **CONFIRMED binding; type 2 STRONG render-family candidate** | all 55 records matrixed; bounded ELF analysis anchors the MTL loader and GS state builder but stock Ghidra's R5900 gaps prevent their data-flow join | 3 |
...
3. **P0c — material semantics:** property type 2 is a strong alpha/render-family selector. V4 explains CLOUD's color/intensity rings but disproves varying vertex alpha as its transparency source. The executable anchors the MTL deserializer and paired GS `TEST`/`ALPHA`/`ZBUF` builder, but exact type-2 state remains blocked by missing R5900 `LQ`/`SQ`/COP2 analysis support.
