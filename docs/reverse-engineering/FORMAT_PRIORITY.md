...
3. **P0c — material semantics:** property type 2 is a strong alpha/render-family selector. V4 explains CLOUD's color/intensity rings but disproves varying vertex alpha as its transparency source. Exact type-2 GS blend operands/fixed factor/depth-write/order state are now the bounded P0 blocker.
