...
A full textured LEVEL00 export was subsequently completed as a local validation artifact. Coordinates, winding, UV orientation, texture orientation, and layout remain coherent. It exposed two independent material-state questions: glTF's default single-sided behavior culls many otherwise coherent surfaces, and conservative opaque rendering makes the partial-alpha `CLOUD` shell occlude the world. A local two-sided diagnostic removes the systemic holes without changing geometry, which supports the confirmed topology while leaving native per-material culling unknown. Alpha blend/mask behavior likewise remains unknown. These findings do not change the coordinate, winding, or source-V conclusions.
