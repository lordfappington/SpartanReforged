...
A full textured LEVEL00 export was subsequently completed as a local validation artifact. Coordinates, winding, UV orientation, texture orientation, and layout remain coherent. It exposed two independent material-state questions: glTF's default single-sided behavior culls many otherwise coherent surfaces, and conservative opaque rendering makes the `CLOUD` shell occlude the world. A later property-matrix study corrected the texture classification: `CLOUD` has fully opaque decoded alpha, not partial alpha. The PS2 GS has no cull bit; an opt-in all-double-sided export removes the systemic holes. MTL child type 2 strongly correlates with alpha/render families, but exact native blend equations, alpha-test threshold, and any pre-GS culling remain unresolved. These findings do not change the coordinate, winding, or source-V conclusions; see [MODELS_MTL_RENDER_SEMANTICS.md](MODELS_MTL_RENDER_SEMANTICS.md).

The subsequent V4-8 survey also leaves these visual conventions unchanged. Its first three bytes are likely color/light modulation and its fourth byte is globally `0x80`. An opt-in glTF-safe vertex-color diagnostic preserves CLOUD's complete `/128` gradient because its values stay below 128; native world values above 128 are clamped only at the glTF `COLOR_0` boundary and remain raw in the parser. This is an attribute/render-semantic question, not a coordinate, winding, or UV correction. See [MODELS_V4_ATTRIBUTES.md](MODELS_V4_ATTRIBUTES.md).
