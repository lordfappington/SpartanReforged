...
That exporter is validated across the complete LEVEL00 MODELS family. BIN supplies geometry/material IDs, MTL supplies ordered identities/resource references, and AAB supplies static provenance/selection without transforming positions. Descriptor 118 confirms coordinates/winding; descriptor 5 confirms source V and native PSMT4 image linkage. All 30 unique strongly bound geometry TIM2 textures now decode exactly against Noesis. See [MODELS_EXPORT_PIPELINE.md](MODELS_EXPORT_PIPELINE.md) and [MODELS_VISUAL_CONVENTIONS.md](MODELS_VISUAL_CONVENTIONS.md). Remaining family blockers are exact MTL sampler/material semantics, seven unresolved material bindings, and V4-8.
