...
That exporter is validated across the complete LEVEL00 MODELS family. BIN supplies geometry/material IDs, MTL supplies ordered identities/resource references, and AAB supplies static provenance/selection without transforming positions. Descriptor 118 additionally confirms source coordinates/source winding and native PSMT4 image linkage. See [MODELS_EXPORT_PIPELINE.md](MODELS_EXPORT_PIPELINE.md) and [MODELS_VISUAL_CONVENTIONS.md](MODELS_VISUAL_CONVENTIONS.md). The next family blocker is a directional V-orientation test, followed by exact MTL sampler/material semantics.
