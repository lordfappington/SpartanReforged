...
`MenuNavigation` is a floating left-aligned text stack with no button rectangles. `SelectionMarker` is a 68×22-design-pixel rendered ornament measured from the approved selected-state concept and placed 17 design pixels left of the selected label. Its elongated rear wing, central diamond, shorter forward point, separately lit/recessed planes, cream-gold ridge, fine rim, restrained weathering and weaker warm halo are supersampled at 4×. Unselected and locked labels share a restrained dimensional material construction. The selected label deliberately uses a different, internally illuminated amber treatment rather than imitating the physical bronze title material.
