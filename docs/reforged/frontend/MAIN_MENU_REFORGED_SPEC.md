...
`MenuNavigation` is a floating left-aligned text stack with no button rectangles. `SelectionMarker` is a compact rendered bronze spearhead placed immediately left of the selected label's optical centre. Its supersampled construction has distinct upper and recessed lower planes, a cream-gold central ridge, fine rim highlights, restrained weathering and a weaker warm halo than the selected text. Unselected and locked labels share a restrained dimensional material construction. The selected label deliberately uses a different, internally illuminated amber treatment rather than imitating the physical bronze title material.
