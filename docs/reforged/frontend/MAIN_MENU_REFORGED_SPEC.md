...
`Logo` accepts an independent transparent logo and optional glint/flare layer. The wireframe uses a labelled rectangle. Runtime scaling preserves aspect ratio, respects `maxWidth`, and never enlarges beyond the available safe zone. Final source specifications live in the asset contract.
